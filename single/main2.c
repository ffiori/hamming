...
extern void prep(unsigned char *P, size_t m, size_t k, size_t q);
extern size_t exec(unsigned char *T, size_t n, size_t k, size_t q);
...
int main(int argc, char *argv[])
{
    ...
    k = q = 0;
    p = e = 1;
    while ((c = getopt(argc, argv, "k:p:e:q:")) != -1) {
        switch (c) {
    ...
    if (optind != argc-1 || e < 1 || p < 1 || k < 0 || q < 0) {
        fprintf(stderr, "Usage: %s [-k err] [-q] [-p preps] [-e execs] "
                        "textfile <patterns\n", argv[0]);
        return 1;
    }
...
next:   i = j+1;
    ...
    fprintf(stdout, " k=%d q=%d occ=%lu", k, q, (unsigned long)occ);
