...
void prepPartExact(PartExact * P, uchar * pattern, int m, int k)
{
    ...
    for(i=0; i<npats; i++){
        ...
        //printf("P[i].cut.ps[0] %s, P[i].cut.j %d, P[i].cut.len[0] %d\n",P[i].cut.ps[0], P[i].cut.j, P[i].cut.len[0]);
        int minlen=P[i].cut.len[0];
        int j;
        for(j=1; j<P[i].cut.j; j++) minlen = min(minlen,P[i].cut.len[j]);
    ...
    typedef struct
       ...
       */
        ...
        if(m>32) prepVer(&P[i].dyn, pattern+i*(m+1), m, k);

        /* precompute some values */
        P[i].back = m - P[i].cut.len[k];
        P[i].forth = m + 1;
    }
    
    if(m<=32) ans_prep(pattern,m,k,npats);
}
                     ...
                     uchar * _, int m, int k)
{
    ...
    for(patnow=0; patnow<npats; patnow++) A[patnow].dyn.lastpos = -1; 
    ...
    if(m<=32){
        ...
        while(true) {
            ...
            */
            ...
            if (i <= A[patnow].dyn.lastpos) i = A[patnow].dyn.lastpos+1; 
            while(i < _pe_f-_vm) { 
                _vmm = 0; 
...
void prepPartExact(PartExact * P, uchar * pattern, int m, int k)
{
    ...
    for(i=0; i<npats; i++){
        ...
        /* Delete repeated subpatterns (little speedup maybe?). */
		int j;
		LIST *freeme = NULL;
        for(j=0; j<P[i].cut.j; j++){
			uchar *p1 = P[i].cut.ps[j];
			int k, len1 = strlen(p1);
			for(k=j+1; k<P[i].cut.j; k++){
				uchar *p2 = P[i].cut.ps[k];
				bool equal = true;
				int pos;
				int lim = min(len1, strlen(p2));
				for(pos=0; pos<lim; pos++) if(p1[pos] != p2[pos]){
					equal = false;
					break;
				}
