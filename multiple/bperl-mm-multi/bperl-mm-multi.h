...
#define MAXPATS 112345          //max number of patterns (not subpatterns)
...
#define W (8*sizeof(long))      /* word length */

typedef unsigned long Tsection[Sigma];  /* T for a single cell */
...
typedef struct ret
{
    ...
    int patt, subpatt;
    int pos;
} RetList;                      //return value of mepsm_exec

RetList *searchAns[MAXPATS];
RetList *lastNode[MAXPATS];
...
void searchVerV (VerData * D, uchar * text, int n, int fin, Tcode code, void *ptr);

        /* similar, but no end point and no reverification test.
           here it is necessary to use initVerprog */

void searchVer (VerData * D, uchar * text, int n, Tcode code, void *ptr);
