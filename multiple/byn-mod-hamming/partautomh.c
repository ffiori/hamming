...
int searchFPartAutomH (PartAutom * M, register uchar * text, int from, int to, int *matches)
{
    ...
    while (true) {              /* update matrix */
        ...
        if (active == 0) {      /* use first autom as filter */
            ...
            while (true) {
                ...
                D0 = ((D0 << 1) | m1) & ((D0 << p3) | m3) & (((x + m1) ^ x) >> 1) & Din;
                if (~D0 & G0) { /* active? */
                    if (!(D0 & Gl0))
                        break;  /* found */
                }
        ...
        for (j = 0; j <= active; j++) {
            ...
            newD[j] = (((D[j] << 1) & ((D[j] << p3) | (D[j + 1] >> p5))) | m1)
                & (((x + m1) ^ x) >> 1)
                & Din;
        }
        aux = D;
...
int searchXPartAutomH (PartAutom * M, register uchar * text, int from, int to, int *matches)
{
    register Tsection *T = M->T.d1;
    register mask *D = M->D.d1;
    register mask *newD = M->newD.d1;
    ...
    while (true) {              /* update matrix */
        ...
        /* check if lower-right state is active */
        if (!(D[J] & Glast[J])) {       /* clear last diagonal */
            D[J] |= m2;
            /* fill match */
            if (n > 0) {        /* otherwise it was just retraining */
                if (n <= to) {
...
int searchPartAutomH (PartAutom * M, register uchar * text, int from, int to, int *matches)
{
    register Tsection *T = M->T.d1;
    register mask *D = M->D.d1;
    register mask *newD = M->newD.d1;
...
int searchVPartAutomH (PartAutom * M, register uchar * text, int from, int to, int *matches)
{
    register Tsection *T = M->T.d1;
    register mask *D = M->D.d1;
    register mask *newD = M->newD.d1;
