...
static void buildV (PartMixedV *P, Cut *cut, uchar *pattern, int m, int k, 
		    int j, int *ver, int offset, int c)
   ...
   { P->large = (j > 1);
     ...
     if (P->large)  /* compound */
        { P->one = (void*)malloc (sizeof(PartMixedV));
          ...
          buildV (P->one,cut,pattern,ver[j/2]-ver[0],((j/2)*k)/j,
                 j/2,ver,0,c);
          buildV (P->two,cut,pattern,ver[j]-ver[j/2],((j-j/2)*k)/j,
                 j-j/2,ver+j/2,ver[j/2]-ver[0],c);
        }
   }

int prepPartMixed (PartMixed *P, uchar **pattern, int r, int m, int k, int jj, 
			int rp, int c)
   ...
   { int rr,d,j,p,i,ksj,jjj=jj;
     ...
     for (rr=0;rr<r;rr++) { P->vfrom[rr] = j; P->vto[rr] = 0; }
     p = 0;
     for (i=0; i<j; i++)
	  ...
	  while (rr < (p-1+jjj-1)/jjj)
	    { if (P->vfrom[rr] > i) P->vfrom[rr] = i;
	      if (P->vto[rr] < i) P->vto[rr] = i;
	      rr++;
	    }
        }
     freeCut(&cut);
...
void initPartMixed (PartMixed *P)
   ...
   { int i;
     for (i=0;i<P->j;i++) initPartAutom (P->D+i);
     for (i=0;i<P->r;i++) initPartMixedV (P->V+i);
   }
...
void freePartMixed (PartMixed *P)
   ...
   { int i;
     for (i=0;i<P->j;i++) freePartAutom (P->D+i);
     for (i=0;i<P->r;i++) freePartMixedV (P->V+i);
     free (P->D); free (P->V);
     free (P->vfrom); free(P->vto);
   }

int searchPartMixedV (PartMixedV *P, register uchar *text, int from, int to, 
			int *matches, int *lmatches, int lcount)
   ...
   { register int i1,i2;
	...
	{ m1 = (int*)malloc ((to-from)*sizeof(int));
	  ...
	  if (ff1 < ff2) 
	    ...
	    { tt = ff + 2*mPk; if (tt > to) tt = to;
		   ...
		   if (nff <= tt) { tt = nff + 2*mPk; if (tt > to) tt = to; }
                   else break;
		 }
              initPartAutom (&P->dynaut);
              count += searchPartAutom (&P->dynaut,text,ff,tt,
				matches == NULL ? NULL : matches+count);
	      ff = nff;
            }
          free (m1); free (m2);
	  return count;
	...
	{ m1 = lmatches;
	  ...
	  o2 = mPk - from;
          while (i1<c1)
             { ff = m1[i1]-o2; tt = m1[i1]+o1;
               while (++i1 < c1)
                 { if (m1[i1]-o2 <= tt) tt = m1[i1]+o1;
                   else break;
                 }
               if (tt > to) tt = to;
               initPartAutom (&P->dynaut);
...
int searchPartMixed (PartMixed *P, uchar *text,
   ...
   { int i,j;
     ...
     for (j=0; j<P->r; j++)
	   ...
	   int min,rmin;
	   count1 = 0;
	   for (r=0;r<P->j;r++) pos[r] = 0;
	   while (1)
	     { min = to+1;
	       for (r=0;r<top;r++)
		   { if ((pos[r] < lcount1[r]) && (lmatches1[r][pos[r]] < min))
			{ min = lmatches1[r][pos[r]]; rmin = r; }
		   }
	       if (min > to) break;
		  ...
		  matches1[count1++] = lmatches1[rmin][pos[rmin]];
           ...
           count += searchPartMixedV (P->V+j, text, from, to, 
			matches == NULL ? NULL : matches+count,
                        matches1, count1);
         }

     for (i=0;i<P->j;i++) free(lmatches[i]);
     free (lmatches); free (lcount); free(matches1); free (pos);
     return count;
   }
