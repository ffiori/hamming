...
void searchMulti (Multi *M, uchar *text, int from, int to, int **matches,
		  int *count)
		   ...
		   if (pos > 0) 
		      { if (pos <= to) 
			   while (i != -1) /* many equal patterns */
			     { if ((count[i] % 1000) == 0)
				  matches[i] = realloc (matches[i],
					sizeof(int)*(count[i]+1000));
